...
exports.generateSourceMapTree = function (analysisContext) {
  ...
  function _findImportItems(ast, componentPath) {
    ...
    function dealImports(importPath, importApis) {
      ...
      if (analysisContext.componentApiMap[absolateComponentPath]) {
        ...
        analysisContext.componentApiMap[`${absolateComponentPath}.vue`]
      ) {
        currentChildrenPath.push({
          path: `${absolateComponentPath}.vue`,
          importApis,
        });
      } else if (
        analysisContext.componentApiMap[`${absolateComponentPath}/index.vue`]
      ) {
        currentChildrenPath.push({
          path: `${absolateComponentPath}/index.vue`,
          importApis,
        });
      }
    }
  ...
  function generateSourceMapTree(context) {
    ...
    const allComponentPath = Object.keys(context.componentApiMap);
    const { program: componentProgram, realFilePathTmpTsFilePathMap } =
      createProgram(allComponentPath, true);
    const componentTree = _buildTree(
      allComponentPath.map((item) => ({
        path: item,
        importApi: [],
      })),
      componentProgram,
